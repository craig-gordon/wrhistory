...
export default class GamePage extends React.Component {
  ...
  componentDidMount() {
    axios
      .post('/api/chartPage/document', {
        uriEndpoint: this.props.location.pathname.slice(6)
      })
      .then(res => {
  ...
  listenForArrowKeys(e) {
    ...
    if (
      e.keyCode === 39 &&
      this.state.selectedChartPoint !== records.length - 1
    ) {
      ...
      <GamePageContainer>
        <StyledTabs
          ...
          onTabClick={e => {
            if (e === '3') {
              ...
              let recordsWithNullsConverted = doc.records.map(record =>
                convertNullsToEmptyStrs(record)
              );
              let newDoc = { ...doc, records: recordsWithNullsConverted };
              let stateToPipe = {
                currentPage: 1,
                totalPages: doc.records.length,
                chartType: doc.type,
                workingDoc: newDoc
              };
        ...
        >
          ...
          <TabPane tab={<i className='fas fa-table' />} key='2'>
            <TabContentsContainer>
              <RecordsTable document={this.state.document} />
            </TabContentsContainer>
          </TabPane>
          <TabPane
